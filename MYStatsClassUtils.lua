...
MYS_GenerateTalentsIndexMap = function()
...
function MYS_GetTalentInfo(tabIndex, talentIndex)
...
function MYS_GetMageCritStatsFromTalents()
	...
	local spellRank = select(5, MYS_GetTalentInfo(1, 15));
	...
	spellRank = select(5, MYS_GetTalentInfo(2, 13));
	if (spellRank > 0) and (spellRank <= 3) then
...
function MYS_GetMageSpellHitFromTalents()
	local arcaneHit = 0;
	local frostFireHit = 0;

	-- Arcane Focus
	local spellRank = select(5, MYS_GetTalentInfo(1, 2));
	arcaneHit = spellRank * 2; -- 2% for each point

	-- Elemental Precision
	spellRank = select(5, MYS_GetTalentInfo(3, 3));
...
function MYS_GetWarlockSpellHitFromTalents()
	local afflictionHit = 0;

	-- Suppression
	local spellRank = select(5, MYS_GetTalentInfo(1, 1));
...
function MYS_GetWarlockCritStatsFromTalents()
	-- the spell rank is equal to the value
	local devastationCrit = select(5, MYS_GetTalentInfo(3, 7));

	return devastationCrit;
end

-- returns the combined crit stats from Holy Specialization and Force of Will
function MYS_GetPriestCritStatsFromTalents()
...
function MYS_GetPriestBonusHealingModifierFromTalents()
	-- Spiritual Healing
	local spellRank = select(5, MYS_GetTalentInfo(2, 15));
	return spellRank * 0.02;
end

-- returns the crit bonus from Holy Power
function MYS_GetPaladinCritStatsFromTalents()
	-- Holy Power (1, 2, 3, 4, 5)%
	local spellRank = select(5, MYS_GetTalentInfo(1, 13));

	return spellRank;
end

-- returns the defense bonus from the Anticipation Prot talent
local function MYS_GetPaladinDefenseFromTalents()

    local defense = 0;
    local defenseTable = { 2, 4, 6, 8, 10 };

    -- Anticipation (2, 4, 6, 8, 10)%
    local spellRank = select(5, MYS_GetTalentInfo(2, 9));
    if (spellRank > 0) and (spellRank <=5) then
...
function MYS_GetPaladinImprovedBoWModifier()
	-- Improved Blessing of Wisdom
	local spellRank = select(5, MYS_GetTalentInfo(1, 10));

	return spellRank * 0.1;
end

-- Checks if spellId is Blessing of Wisdom
function MYS_IsBoWSpellId(spellId)
...
-- returns the defense bonus from the Anticipation Prot talent
local function MYS_GetWarriorDefenseFromTalents()
    
    local defense = 0;
    local defenseTable = { 2, 4, 6, 8, 10 };

    -- Anticipation (2, 4, 6, 8, 10)%
    local spellRank = select(5, MYS_GetTalentInfo(3, 2));
    if (spellRank > 0) and (spellRank <=5) then
		defense = defenseTable[spellRank];
	end
...
function MYS_GetDefenseFromTalents(unit)
    
	local defense = 0;
	local unitClassId = select(3, UnitClass(unit));

    if (unitClassId == MYS_PALADIN_CLASS_ID) then
        defense = MYS_GetPaladinDefenseFromTalents();
    elseif (unitClassId == MYS_WARRIOR_CLASS_ID) then
        defense = MYS_GetWarriorDefenseFromTalents();
    end
...
function MYS_GetShapeshiftForm()
	local shapeIndex = 0;
	local shapeId=0;

	for possibleForm=1, GetNumShapeshiftForms() do
		local active,_,id=select(2,GetShapeshiftFormInfo(possibleForm))
		if active then
...
function MYS_GetShamanTidalMasteryCrit()
	-- Tidal Mastery (Nature/Lightning)
	local spellRank = select(5, MYS_GetTalentInfo(3, 11));
	return spellRank;
end

-- ITEMS AND ENCHANTS RELATED
function MYS_GetMP5ModifierFromTalents(unit)
    local unitClassId = select(3, UnitClass(unit));
	local spellRank = 0;

	if unitClassId == MYS_PRIEST_CLASS_ID then
		-- Meditation
        spellRank = select(5, MYS_GetTalentInfo(1, 8));
	elseif unitClassId == MYS_MAGE_CLASS_ID then
		-- Arcane Meditation
		spellRank = select(5, MYS_GetTalentInfo(1, 12));
	elseif unitClassId == MYS_DRUID_CLASS_ID then
		-- Reflection
        spellRank = select(5, MYS_GetTalentInfo(3, 6));
	end
...
function MYS_GetMP5ModifierFromSetBonus(unit)
	local unitClassId = select(3, UnitClass(unit));
	local modifier = 0;
	
	-- not Druid or Priest
	if unitClassId ~= MYS_DRUID_CLASS_ID and unitClassId ~= MYS_PRIEST_CLASS_ID then
		return modifier;
	end
...
function MYS_GetShamanT2SpellCrit(unit)
...
function MYS_GetHolyCritFromBenediction(unit)
...
function MYS_GetBlockValueFromWarriorZGEnchants(unit)
	local blockValue = 0;

	if MYS_HasEnchant(unit, INVSLOT_HEAD, 2583) then
		blockValue = blockValue + 15;
	end

	if MYS_HasEnchant(unit, INVSLOT_LEGS, 2583) then
		blockValue = blockValue + 15;
	end
...
function MYS_GetMp5FromPriestZGEnchants(unit)
	local mp5 = 0;

	if MYS_HasEnchant(unit, INVSLOT_HEAD, 2590) then
		mp5 = mp5 + 4;
	end

	if MYS_HasEnchant(unit, INVSLOT_LEGS, 2590) then
		mp5 = mp5 + 4;
	end
